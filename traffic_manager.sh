...
_tm_ensure_stats_client() {
    ...
    command -v unzip >/dev/null 2>&1 || { echo "unzip is required to install the traffic statistics client" >&2; return 1; }
    ...
    if command -v curl >/dev/null 2>&1; then curl -LfsS "$url" -o "$tmp/xray.zip" || { rm -rf "$tmp"; return 1; }
    elif command -v wget >/dev/null 2>&1; then wget -q "$url" -O "$tmp/xray.zip" || { rm -rf "$tmp"; return 1; }
    else rm -rf "$tmp"; return 1; fi
    unzip -qo "$tmp/xray.zip" xray -d "$tmp" || { rm -rf "$tmp"; return 1; }
    mkdir -p "$(dirname "$TM_XRAY_CLIENT_BIN")"
    mv "$tmp/xray" "$TM_XRAY_CLIENT_BIN" && chmod +x "$TM_XRAY_CLIENT_BIN"
    rm -rf "$tmp"
}
...
_tm_set() {
    local core="$1" tag="$2" mode="$3" limit="$4" reset_day="${5:-}" key pattern period=""
    ...
    pattern="^${tag}(-hop-[0-9]+)?$"
    _tm_ensure_stats_client || return 1
    if [ "$core" = singbox ]; then _tm_ensure_singbox_api "$tag" || return 1; else _tm_ensure_xray_api || return 1; fi
    _tm_atomic_jq '. + {($k): ((.[$k] // {}) + {core:$c,tag:$t,mode:$m,limit_bytes:($l|tonumber),used_bytes:(.[$k].used_bytes // 0),reset_day:(if $r=="" then null else ($r|tonumber) end),period_key:(if $p=="" then null else $p end),member_pattern:$pat,last_uplink:(.[$k].last_uplink // 0),last_downlink:(.[$k].last_downlink // 0),disabled:(.[$k].disabled // false),disabled_reason:(.[$k].disabled_reason // null)})}' \
        --arg k "$key" --arg c "$core" --arg t "$tag" --arg m "$mode" --arg l "$limit" --arg r "$reset_day" --arg p "$period" --arg pat "$pattern" || return 1
    if [ "$(jq -r --arg k "$key" '.[$k].disabled // false' "$TM_STATE_FILE")" = true ] && [ "$(jq -r --arg k "$key" '.[$k].used_bytes < .[$k].limit_bytes' "$TM_STATE_FILE")" = true ]; then
        _tm_restore_node "$core" "$tag" && _tm_restart_core "$core"
    fi
...
_tm_edit_identity() {
    ...
    _tm_atomic_jq 'if .[$ok] then . + {($nk): (.[$ok] | .tag=$nt | .member_pattern=$pat | .saved_inbounds=((.saved_inbounds // []) | map(.tag=(.tag | gsub($ot;$nt)) | if has("listen_port") and .listen_port == ($op|tonumber) then .listen_port=($np|tonumber) elif has("port") and .port == ($op|tonumber) then .port=($np|tonumber) else . end)) | .last_uplink=0 | .last_downlink=0)} | del(.[$ok]) else . end' \
...
_tm_port_in_use() {
    _tm_init || return 1
    local core="$1" port="$2"
    jq -e --arg c "$core" --argjson p "$port" 'any(.[]; .core == $c and any((.saved_inbounds // [])[]?; (.listen_port // .port // 0) == $p))' "$TM_STATE_FILE" >/dev/null
}
...
_tm_acquire_lock() { mkdir "$TM_LOCK_DIR" 2>/dev/null; }
_tm_release_lock() { rmdir "$TM_LOCK_DIR" 2>/dev/null || true; }
...
_tm_ensure_singbox_api() {
    ...
    [ -f "$config" ] || return 1
    jq --arg t "$tag" '.experimental = (.experimental // {}) | .experimental.v2ray_api = ((.experimental.v2ray_api // {}) + {listen:(env.TM_SINGBOX_API_LISTEN // "127.0.0.1:10086"), stats:((.experimental.v2ray_api.stats // {}) + {enabled:true,inbounds:((.experimental.v2ray_api.stats.inbounds // []) + (if $t == "" then [] else [$t] end) | unique)})})' "$config" > "$tmp" && mv "$tmp" "$config"
}
...
_tm_restart_core() {
    ...
    if [ "$core" = singbox ]; then
        [ -n "${TM_RESTART_SINGBOX_CMD:-}" ] && eval "$TM_RESTART_SINGBOX_CMD" || systemctl restart sing-box 2>/dev/null || true
    else
        [ -n "${TM_RESTART_XRAY_CMD:-}" ] && eval "$TM_RESTART_XRAY_CMD" || systemctl restart xray 2>/dev/null || true
    fi
}

_tm_disable_node() {
    local core="$1" tag="$2" config tmp key
    config=$(_tm_config_path "$core"); [ -f "$config" ] || return 1
    key=$(_tm_key "$core" "$tag"); tmp="${config}.tmp.$$"
    jq --arg t "$tag" '.inbounds as $all | ([ $all[] | select(.tag == $t or (.tag | startswith($t + "-hop-"))) ]) as $saved | {config:(. | .inbounds = ([ $all[] | select(.tag != $t and ((.tag | startswith($t + "-hop-")) | not)) ])), saved:$saved}' "$config" > "${tmp}.json" || return 1
    jq '.config' "${tmp}.json" > "$tmp" && mv "$tmp" "$config" || { rm -f "$tmp" "${tmp}.json"; return 1; }
    local saved; saved=$(jq -c '.saved' "${tmp}.json" 2>/dev/null || echo '[]'); rm -f "${tmp}.json"
    _tm_atomic_jq ' .[$k].disabled=true | .[$k].disabled_reason="quota" | .[$k].saved_inbounds=$s ' --arg k "$key" --argjson s "$saved"
}
...
_tm_restore_node() {
    ...
    config=$(_tm_config_path "$core"); [ -f "$config" ] || return 1
    key=$(_tm_key "$core" "$tag"); tmp="${config}.tmp.$$"
    local saved; saved=$(jq -c --arg k "$key" '.[$k].saved_inbounds // []' "$TM_STATE_FILE")
    [ "$saved" != '[]' ] || return 0
    jq --argjson s "$saved" '.inbounds = ((.inbounds // []) + $s | unique_by(.tag))' "$config" > "$tmp" && mv "$tmp" "$config" || return 1
    _tm_atomic_jq ' .[$k].disabled=false | .[$k].disabled_reason=null | .[$k].saved_inbounds=null | .[$k].last_uplink=0 | .[$k].last_downlink=0 ' --arg k "$key"
}
...
_tm_check() {
    ...
    while IFS=$'\t' read -r key core tag mode limit used saved_period; do
        ...
        local counters; counters=$(_tm_query_counters "$core" "$tag") || continue
    ...
    [ "$changed_singbox" = true ] && _tm_restart_core singbox
    [ "$changed_xray" = true ] && _tm_restart_core xray
    _tm_release_lock
}

_tm_install_schedule() {
    local script="${TM_SCRIPT_PATH:-$0}" cron_file="${TM_CRON_FILE:-/etc/cron.d/singbox-lite-traffic}"
